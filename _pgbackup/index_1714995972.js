...
function showSlide(index) {
    ...
    Array.from(paginationContainer.children).forEach((paginationItem, i) => {
        ...
        if (i === index) {
            div.classList.add('scale-110', 'bg-soft_orange-500'); // Make the active item larger and darker
            div.classList.remove('bg-soft_orange-300'); // Remove the lighter color
        } else {
            div.classList.remove( 'scale-110', 'bg-soft_orange-500'); // Make the non-active items their original size and color
